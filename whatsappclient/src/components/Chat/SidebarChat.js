...
export default function SidebarChat(props) {
  ...
  const lastMessage = (msg) => {
    if (msg === undefined || msg.Content === undefined || msg.Content === null) return "";
    else return (
    <span className="sidebar__overflow">{msg.Content.length<43 ? msg.Content : msg.Content.substring(0,43) + "..."}</span>
  )}
  ...
  return (
    <div
      ...
      onClick={() => { props.setActiveContact(props.contact) }}
    >
      <span>
